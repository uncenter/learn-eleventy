...
First of all, we’ve added a named collection to our Eleventy config called **“work”**. The parameter that is passed into our function is an instance of the [Eleventy collections API](https://www.11ty.dev/docs/collections-api/).

Using the [`getFilteredByGlob` method](https://www.11ty.dev/docs/collections-api/#get-filtered-by-glob-glob) of that collection API, we’re telling Eleventy to find all **markdown files** that live inside of the `eleventy-from-scratch/src/work` folder. This returns an array, so we can use the `sort()` method to compare each work item’s `displayOrder` and sort them accordingly.
...
At the bottom of our configuration file, after the `};` of the `export const config` object, add the following:
