...
Let’s add this to our critical CSS. Open up `eleventy-from-scratch/src/css/critical.scss` and add the following, with the other block `@import` statements:
...
These two new blocks are very specific to work items, so let’s add our last critical CSS. Create a new file in your `css` folder called `work-item.scss` and add the following to it:
...
```njk
{% set pageCriticalStyles = ['work-item.scss'] %}
```
