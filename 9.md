...
If you haven't dealt with `async`/`await` before, don't worry. We use `async` to declare an **asynchronous** function (`async function someName(){...}`). This means that the function will return a [`Promise`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise) object- a placeholder for a value that will be resolved in the future. We then use the `await` keyword inside an `async` function to **pause** the execution of our `async` function until the`Promise` is resolved (which is when we get our data back).
