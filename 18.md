...
With this addition, a page can declare `pageStylesheets` for **non-critical** styles. Here, instead of inlining the CSS like we do with `pageCriticalStyles`, we add a reference to an external stylesheet. Thanks to a handy Eleventy filter, [`inputPathToUrl`](https://www.11ty.dev/docs/filters/inputpath-to-url/), we only need to specify where the CSS files are in our input directory, and Eleventy will fill in where they end up. None of these CSS files exist yet, but we’ll make them soon.
