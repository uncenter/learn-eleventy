...
Nice and easy. Let’s add that to our critical CSS. Open up `eleventy-from-scratch/src/css/critical.scss` and add the following, with the other block `@import` statements:
...
Create a new file called `eleventy-from-scratch/src/css/blocks/_post-list.scss` and add the following to it:
...
```njk
{% set pageCriticalStyles = ['page.scss'] %}
```
...
Right, let’s add that to our page critical CSS. Open up `eleventy-from-scratch/src/css/page.scss` and right at the bottom, add the following:
...
Let’s add that to our critical page CSS. Open up `eleventy-from-scratch/src/css/page.scss` and add the following **right at the bottom**:
