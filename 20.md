...
We’re using the [official Eleventy Image plugin, `@11ty/eleventy-img`](https://www.11ty.dev/docs/plugins/image/). Specifically, the Image plugin's [HTML transform setup](https://www.11ty.dev/docs/plugins/image/#html-transform) which post-processes the `<img>` and `<picture>` tags in our HTML.
