...
>
> You can see this utility in `eleventy-from-scratch/src/css/utilities/_visually-hidden.scss`.

## Wiring it all up

Just like in the last lesson, we need to import this CSS, so open up `eleventy-from-scratch/src/css/critical.scss` and add the following, with the other block `@import` statements:
