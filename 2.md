...
This `package.json` file does twofold; firstly, `"type": "module"` gives our package the module type so that Node knows to parse it as an [ES module](https://nodejs.org/api/esm.html#introduction) (the most standard format for JavaScript modules, as opposed to the older CommonJS). Secondly, the `start` and `build` scripts give us more convenient ways of running Eleventy (more on that in the next section).
...
> [!NOTE]
> You might have noticed that the install command specified the `@11ty/eleventy@v3` package. This is in the format of `<name>@<version>`, indicating that the package is `@11ty/eleventy`, and we want version `v3` of it. We will continue to see this sort of "pinning" of dependencies to major versions throughout the course, as we want you to experience this course as it was intended.
...
The command `npx eleventy --serve` uses the `npx` program, a part of the `npm` command-line interface. `npx` enables us to run a command from a local or remote npm package. In this case, we use the program `eleventy`, which comes from having the Eleventy dependency already installed locally in our project. We could have also used the terminal command `npx @11ty/eleventy --serve`, which would do the exact same thing even if we didn't already have Eleventy installed. Pretty sweet!
