...
Since we are going to include this critical CSS inline on our pages, we don't need it to be generated in the output directory. In your `scss` folder, create a file called `critical.json` and add the following inside:
...
Like our directory data files `work.json` for the `work/` collection and `posts.json` for the `posts/` collection, because `critical.json` and `critical.scss` share the same file slug (the `critical` part before the file extension), Eleventy applies the data from the JSON file to our Sass template. This is called a [Template Data File](https://www.11ty.dev/docs/data-template-dir/).

In this case, our template data file disables the `permalink` property, [telling Eleventy to skip writing this file to the output directory](https://www.11ty.dev/docs/permalinks/#skip-writing-to-the-file-system).
...
```njk
<style>{% renderFile "./src/scss/critical.scss" %}</style>

{# Add facility for pages to declare an array of critical styles #}
{% if pageCriticalStyles %}
	{% for item in pageCriticalStyles %}
		<style>{% renderFile "./src/scss/" + item %}</style>
	{% endfor %}
{% endif %}
```
