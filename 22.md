...
<a href="/static/eleventy-from-scratch-front-end-build-starter-files.zip" class="button">Download starter files.</a>

Download the starter files above and **completely replace** `eleventy-from-scratch/src/css` with the `css` folder in the ZIP archive that you downloaded.

Your `eleventy-from-scratch/src/css` folder should now look like this:
...
To make working with the CUBE CSS methodology easier, I created a Sass tool called [Gorko](https://github.com/Andy-set-studio/gorko). It generates utility functions for us based on a configuration file. This allows us to have a single source of truth for our CSS.
...
Now that’s installed, open up `eleventy-from-scratch/src/css/critical.scss` and add the following to **the top of the file**:
...
## A quick tour of your css folder

I’m going to quickly show you around the current `css` folder so you know:
...
Take a quick look at `eleventy-from-scratch/src/css/_config.scss`. Let me explain what’s going on.
...
As per the [CUBE CSS methodology](https://cube.fyi), we have **blocks** and **utilities**. [Gorko](https://github.com/hankchizljaw/gorko) and our config have at this point generated most of the utilities, but you’ll see that there are a few set for us in `eleventy-from-scratch/src/css/utilities`.
...
Open up `eleventy-from-scratch/src/css/critical.scss` add the following **after the reset**, on around **line 8**:
