...
Next, we'll get our font assets for Red Hat Display at https://gwfh.mranftl.com/fonts/red-hat-display and repeat the steps. Again, start by making sure the `latin` charset is enabled and _is the only charset enabled_. This time, select only the `regular` amd `900` styles.
...
Lastly, we'll need to use passthrough copy from [lesson 5](/lesson/5) again to copy our font files to the output directory.
