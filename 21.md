...
We’re going to use a handy Eleventy feature called [Transforms](https://www.11ty.dev/docs/config/#transforms). Transforms are a bit like filters where we take the input, pass it though the Transform and then get the transformed output back in return. The Eleventy Image transform itself uses Transforms under the hood, so we already know how powerful they can be.
