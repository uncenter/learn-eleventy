...
In `about.html` we check to see if the `peopleItems` variable is set. If it is, we use it here to loop through it, creating a list of `<figure>`s for each person. Job done!
