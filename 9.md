...
First up, let’s take a look at the data we’re grabbing. In a new browser tab, go to <a href="/static/media.json">/static/media.json</a>. You should see something that looks like this:
	...
	"items": [
		{
			...
			"large": "https://learn-eleventy.pages.dev/static/notepad_large.jpg",
			"medium": "https://learn-eleventy.pages.dev/static/notepad_medium.jpg",
			"small": "https://learn-eleventy.pages.dev/static/notepad_small.jpg",
...
The first thing we need to do is add a library to fetch our JSON file. We could use the standard [`fetch()` Web API](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API) available in Node.js and browsers, but
...
```sh
npm install @11ty/eleventy-fetch@v5
```
