...
The command `npx eleventy --serve` uses the `npx` program, a part of the `npm` command-line interface. `npx` enables us to run a command from a local or remote npm package. In this case, we use the program `eleventy`, which comes from having our local Eleventy dependency already insatlled. We could have also used `npx @11ty/eleventy --serve`, which would do the exact same thing, even if we didn't already have Eleventy installed locally. Pretty sweet!
