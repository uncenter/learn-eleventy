			...
			{% for item in gallery %}
				<figure class="[ gallery__item ] [ flow ]">
					<div class="[ gallery__media ] [ frame ]" data-frame="primary">
						<img src="{{ item.image }}" alt="" aria-hidden="true" class="radius height-auto" />
					</div>
...
Open up `eleventy-from-scratch/src/_data/helpers.js`. At the bottom of the file, after the closing `}` of our last helper `getSiblingContent`, add the following:
