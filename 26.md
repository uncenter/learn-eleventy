...
The first thing we do is flip the `::selection` styles. The global selection styles won’t work because this panel has a dark background.
...
The call to action is used everywhere on the site, so we’re going to add it to our global, critical CSS. Open up `eleventy-from-scratch/src/css/critical.scss` and add the following, with the other block `@import` statements:
...
Let’s add that to our home critical CSS. Open up `eleventy-from-scratch/src/css/home.scss` and right at the end, add the following:
...
Let’s add that to our home critical CSS. Open up `eleventy-from-scratch/src/css/home.scss` and at the end, on **line 13** add the following:
